...
- `default` выбирает модель, настроенную в CLI. Для Codex список моделей и возможности запрашиваются у официального `codex app-server` для конкретного входа, поэтому в интерфейсе показываются модели именно выбранного аккаунта. Если CLI не отвечает, остаётся безопасный список из конфигурации сайта.
...
Если провайдер возвращает исчерпанную квоту во время задачи, runner передаёт это как `rate_limit`. В режиме `Auto` маршрутизатор помечает аккаунт ограниченным, показывает уведомление и запускает продолжение на следующем аккаунте того же runner в том же каталоге проекта. Если уже были изменены файлы, следующий аккаунт сначала видит текущее состояние каталога. Для явно выбранного аккаунта автоматическая передача не выполняется.
