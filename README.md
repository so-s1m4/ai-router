...
- Чат запускает Codex в `read-only`, Antigravity в `plan`; режим задачи использует `workspace-write` и `accept-edits` соответственно. Для задач Antigravity runner добавляет в настройки аккаунта ограниченный список разрешённых headless-команд (`git`, просмотр файлов, `npm ci/install`, `npm run build/test/lint`). Это позволяет CLI работать без интерактивного подтверждения. Для других команд владелец runner может добавить правила `permissions.allow` в `<runner_data>/accounts/<account-id>/home/.gemini/antigravity-cli/settings.json`; `--dangerously-skip-permissions` не используется.
