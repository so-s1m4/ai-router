...
import { z } from 'zod';
import { execute, RunnerError, type Job } from './cli.js';
...
const jobSchema=z.object({jobId:z.string().uuid(),accountId:z.string().uuid(),provider:z.enum(['codex','antigravity']),sessionId:z.string().uuid(),prompt:z.string().min(1).max(40000),model:z.string().max(100),mode:z.enum(['chat','task'])});
