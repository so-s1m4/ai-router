...
import { readAgyModels } from './agy-models.js';
...
export async function accountStatus(provider:ProviderId,home:string,signal:AbortSignal):Promise<AccountStatus>{
  if(provider!=='codex'){
    const defaultGemini:AccountModel[]=[
      {id:'default',label:'По умолчанию аккаунта'},
    ...
    const envAgy=(process.env.AGY_MODELS||'').split(',').map(x=>x.trim()).filter(Boolean).map(id=>({id,label:id}));
    const env=envFor(home,provider);
    const discovered=await readAgyModels(bin.antigravity,home,env,signal).catch(()=>null);
    const map=new Map<string,AccountModel>();
    for(const m of defaultGemini)map.set(m.id,m);
    for(const m of discovered||[])map.set(m.id,m);
    for(const m of envAgy)if(!map.has(m.id))map.set(m.id,m);
...
export async function execute(job:Job,signal:AbortSignal,emit:(e:Event)=>void):Promise<string>{if(signal.aborted)throw new RunnerError('Остановлено','canceled');const home=path.join(root,'accounts',job.accountId,'home'),cwd=workspaceFor(job);await mkdir(home,{recursive:true,mode:0o700});await mkdir(cwd,{recursive:true,mode:0o700});if(signal.aborted)throw new RunnerError('Остановлено','canceled');if(job.mode==='task')job.prompt+=`\n\nPreview hosting on this runner: when the user asks you to create a website, build it in the current project directory and publish its output directory with deploy-preview <directory> [name]. For a live development server use deploy-preview --port <port> [name]. The command prints a URL in the form https://<name>.preview.s1m4.com; include that URL in your answer. Only publish the requested site; never publish account or secret directories. The site remains on this runner and is reachable while the runner is online.`;const command=bin[job.provider];if(!await exists(command)){if(process.env.MOCK_MODE==='false')throw new RunnerError('CLI не установлен','unavailable');const text=`Демо-ответ (${job.provider}): «${job.prompt.slice(0,300)}». Установите CLI в контейнере исполнителя и войдите в аккаунт для реального ответа.`;emit({type:'status',message:'Демо-режим: CLI не установлен'});for(const chunk of text.match(/.{1,24}/gu)||[text]){if(signal.aborted)throw new RunnerError('Остановлено','canceled');emit({type:'delta',text:chunk});await new Promise(r=>setTimeout(r,35));}return text;}
 ...
 let text='',streamed='',status='',failure='';const args=['-p',job.prompt,'--output-format','stream-json','--mode',job.mode==='task'?'accept-edits':'plan'];if(job.mode==='task')args.push('--dangerously-skip-permissions');if(job.model!=='default')args.push('--model',job.model);const effort=(job.reasoning&&job.reasoning!=='default')?job.reasoning:(job.model.startsWith('gemini-3')?'high':undefined);if(effort)args.push('--effort',effort);const result=await runJson(command,args,cwd,home,'antigravity',signal,j=>{if(j.event==='init')emit({type:'status',message:'Antigravity запущен'});else if(j.event==='step_update'){const step=j.step_update||{};if(step.step_type==='tool'){const name=String(step.tool_name||step.tool_info?.name||'Инструмент').slice(0,100),state=String(step.state||'');emit({type:'tool',message:`${name}: ${state==='DONE'?'завершено':'выполняется'}`,data:{state,stepIndex:step.step_index}});}else if(step.step_type==='checkpoint'&&step.state==='DONE')emit({type:'status',message:'Antigravity сохранил этап работы'});else if(step.step_type==='agent_response'&&typeof step.text_delta==='string'){streamed+=step.text_delta;emit({type:'delta',text:step.text_delta});}}else if(j.event==='result'){const r=j.result||j;text=String(r.response||'');status=String(r.status||'');failure=String(r.error||'');if(text&&!streamed)emit({type:'delta',text});if(r.usage)emit({type:'usage',data:r.usage});}},job.mode);if(signal.aborted)throw new RunnerError('Остановлено','canceled');if(result.timedOut)throw new RunnerError('Время выполнения задачи истекло','timeout');if(result.code!==0||status&&status!=='SUCCESS'||!text.trim())throw classify(result.stderr||failure||status||'CLI не вернул ответ');return text;
}
