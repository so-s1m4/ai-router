...
export class App implements OnInit,OnDestroy {
  ...
  onEvent(e:AIEvent){if(e.sessionId!==this.current()?.id)return;if(e.type==='delta'){this.stream.update(s=>s+(e.text||''));this.activeAccount.set(e.data?.accountId||'');}else if(e.type==='status'||e.type==='fallback'||e.type==='checkpoint'||e.type==='handoff_started'||e.type==='handoff_ready'){if(e.type==='handoff_started')this.stream.set('');this.notice.set(e.message||'');this.activeAccount.set(e.data?.accountId||'');}else if(e.type==='error'){this.error.set(e.message||'Ошибка');this.running.set(false);this.runId.set('');this.stream.set('');this.reloadCurrent();}else if(e.type==='completed'){this.running.set(false);this.runId.set('');this.stream.set('');this.notice.set(e.message||'Готово');this.reloadCurrent();}}
