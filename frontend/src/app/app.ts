...
import { LucideArrowUp, LucideArrowUpRight, LucideBot, LucideCopy, LucideInfo, LucideLogOut, LucideMenu, LucideMessageSquare, LucidePencilLine, LucidePlugZap, LucidePlus, LucideRefreshCw, LucideServer, LucideSparkles, LucideSquare, LucideX } from '@lucide/angular';
...
@Component({selector:'app-root',standalone:true,imports:[CommonModule,FormsModule,LucideArrowUp,LucideArrowUpRight,LucideBot,LucideCopy,LucideInfo,LucideLogOut,LucideMenu,LucideMessageSquare,LucidePencilLine,LucidePlugZap,LucidePlus,LucideRefreshCw,LucideServer,LucideSparkles,LucideSquare,LucideX],templateUrl:'./app.html',styleUrl:'./app.css'})
export class App implements OnInit,OnDestroy {
  username=''; password=''; loginName=''; loginError=''; registerMode=signal(false);
  loggedIn=signal(false); page=signal<'chat'|'connections'|'runners'>('chat');
  mobileMenu=signal(false);
  accounts=signal<Account[]>([]); runners=signal<Runner[]>([]); pairing=signal<Pairing|null>(null); sessions=signal<ChatSession[]>([]); current=signal<ChatSession|null>(null);
  ...
  async newSession(){this.mobileMenu.set(false);try{const s=await this.api<ChatSession>('/sessions',{method:'POST'});this.sessions.update(v=>[s,...v]);this.current.set(s);this.page.set('chat');this.stream.set('');this.error.set('');}catch(e){this.error.set((e as Error).message);}}
  async openSession(id:string){if(this.running())return;this.mobileMenu.set(false);try{this.current.set(await this.api<ChatSession>('/sessions/'+id));this.stream.set('');this.error.set('');this.page.set('chat');}catch(e){this.error.set((e as Error).message);}}
  showPage(page:'chat'|'connections'|'runners'){this.page.set(page);this.mobileMenu.set(false);if(page==='runners')this.refreshRunners();}
  connect(){this.socket?.disconnect();this.socket=io({path:'/socket.io',transports:['websocket']});this.socket.on('ai:event',(e:AIEvent)=>this.onEvent(e));this.socket.on('accounts:changed',(a:Account[])=>this.accounts.set(a));this.socket.on('connect_error',()=>this.error.set('Соединение с сервером потеряно'));}
