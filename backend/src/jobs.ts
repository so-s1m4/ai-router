...
export function attachJobHandlers(namespace:Namespace){namespace.on('connection',socket=>{const runnerId=socket.data.runnerId as string;
 socket.on('job:event',(raw:unknown)=>{const p=z.object({jobId:z.string().uuid(),type:z.enum(['status','delta','tool','usage']),text:z.string().max(200000).optional(),message:z.string().max(1000).optional(),data:z.record(z.unknown()).optional()}).safeParse(raw);if(!p.success)return;const job=pending.get(p.data.jobId);if(job?.runnerId===runnerId)job.onEvent(p.data);});
