...
Результаты работы ИИ доступны через кнопку **Файлы** в задаче. Список берётся из рабочего каталога проекта или отдельной задачи. Для выбранного файла кнопка **Ссылка** создаёт адрес со случайным 48-символьным секретом; затем доступны **Скачать** и копирование ссылки. Ссылка действует 7 дней, открывается без входа и требует, чтобы исполнитель был в сети. Файлы до 100 МБ передаются через backend порциями; скрытые файлы, служебные папки и символические ссылки не выдаются. Если файл изменится после создания ссылки, создайте новую.
