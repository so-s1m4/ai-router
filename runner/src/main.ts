...
import { z } from 'zod';
import { accountStatus, execute, RunnerError, type Job, type ProviderId } from './cli.js';
...
const jobSchema=z.object({jobId:z.string().uuid(),accountId:z.string().uuid(),provider:z.enum(['codex','antigravity']),sessionId:z.string().uuid(),projectId:z.string().uuid().optional(),prompt:z.string().min(1).max(40000),model:z.string().max(100),mode:z.enum(['chat','task'])});
async function start(){const d=await device();const socket=io(url+'/runner',{path:'/socket.io',transports:['websocket'],auth:{runnerId:d.id,secret:d.secret},reconnection:true,reconnectionDelay:1000,reconnectionDelayMax:10000});const active=new Map<string,AbortController>();
 ...
 socket.on('job:start',async(raw:unknown,ack?:(r:unknown)=>void)=>{if(!socket.connected)return ack?.({ok:false,error:'Offline'});const parsed=jobSchema.safeParse(raw);if(!parsed.success)return ack?.({ok:false,error:'Invalid job'});const job=parsed.data as Job;if(active.has(job.jobId))return ack?.({ok:false,error:'Duplicate job'});const controller=new AbortController();active.set(job.jobId,controller);ack?.({ok:true});try{const text=await execute(job,controller.signal,e=>{if(socket.connected)socket.emit('job:event',{jobId:job.jobId,...e});});if(socket.connected)socket.emit('job:result',{jobId:job.jobId,ok:true,text});}catch(e){if(socket.connected)socket.emit('job:result',{jobId:job.jobId,ok:false,error:e instanceof Error?e.message:'Ошибка',code:e instanceof RunnerError?e.code:'failed'});}finally{active.delete(job.jobId);}});
 ...
 const refresh=async(accountId:string,provider:ProviderId)=>{const controller=new AbortController();try{const status=await accountStatus(provider,path.join(root,'accounts',accountId,'home'),controller.signal);if(socket.connected)socket.emit('account:status',{accountId,provider,models:status.models,limits:status.limits});}catch(error){if(socket.connected)socket.emit('account:status',{accountId,provider,models:[],error:error instanceof Error?error.message:'Статус не получен'});}};
